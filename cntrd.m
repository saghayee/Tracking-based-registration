function out=cntrd(im_orig,im,mx,sz,interactive)
...
%  MJH 1/14: calculate total brightness in terms of original, pre-processed
% image
...
ind= mx(:,2) > 1.5*sz & mx(:,2) < nr-1.5*sz;
mx=mx(ind,:);
ind= mx(:,1) > 1.5*sz & mx(:,1) < nc-1.5*sz;
mx=mx(ind,:);
...
pts=[];
% generate binary image for area-finding
thresh_val = graythresh(im/max(im(:)));
nL = 0;
while nL < nmx
    b_bin = im2bw(im/max(im(:)),thresh_val);
    L = bwlabeln(b_bin);
    nL = max(L(:));
    thresh_val = thresh_val * 0.9;
end
...
for i=1:nmx
    ...
    tmp_orig=msk.*im_orig((mx(i,2)-r+1:mx(i,2)+r),(mx(i,1)-r+1:mx(i,1)+r));
    %calculate the total brightness, in terms of the original brightness
    norm=sum(sum(tmp));
    norm_orig=sum(sum(tmp_orig));
    ...
    % get region label
    label = L(mx(i,2),mx(i,1));
    if label ~= 0
        area = areas(label).Area;
    else
        continue;
    end
    %concatenate it up
    pts=[pts,[mx(i,1)+xavg-r,mx(i,2)+yavg-r,norm_orig,area,rg]'];
